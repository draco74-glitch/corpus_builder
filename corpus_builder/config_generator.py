...
def detect_source_type(url: str) -> str:
    ...
    Типы должны совпадать с `models.SOURCE_TYPES` и реестром краулеров (C2):
    раньше академические URL (arXiv/DOAJ/Crossref/Wikipedia) определялись как
    «html», и специализированные REST-краулеры не вызывались никогда.
    """
    parsed = urlparse(url)
    ...
    if host.endswith("stackexchange.com") or host in (
        ...
        "stackapps.com", "unix.stackexchange.com", "mathoverflow.net",
    ) or host.endswith("stackoverflow.com"):
        if "/questions/" in path or path.rstrip("/") == "/questions":
            return "stackexchange"

    # --- научные источники: выделяем свои типы, а не «html» (I14) ---
    if host.endswith("wikipedia.org") and "/wiki/" in path:
        return "wikipedia"
    if host.endswith("arxiv.org") and ("/abs/" in path or "/list/" in path or "/pdf/" in path):
        return "arxiv"
    if host.endswith("doaj.org"):
        return "doaj"
    if host in ("doi.org", "dx.doi.org", "api.crossref.org", "search.crossref.org"):
        return "crossref"
...
def seed_crawl_depth(
    ...
    depth = max(depth, 0)
    depth = min(depth, 1000)  # защита от абсурда
...
def build_config(sources: list[dict], output_path: str | Path,
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write("# Generated by corpus_builder.config_generator\n")
...
def from_wikipedia(
    ...
    for category in categories:
        ...
        for member in members:
            ...
            sources.append(make_source(
                article_url,
                # явный тип: вики-краулер отдаёт чищенный REST JSON вместо
                # HTML-скрейпинга (I14)
                "wikipedia",
